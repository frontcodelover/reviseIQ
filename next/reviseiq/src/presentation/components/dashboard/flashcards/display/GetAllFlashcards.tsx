    ...
    <div className='mt-8 flex w-full flex-col items-center justify-center gap-4 px-2 sm:px-4 md:mt-12 md:gap-6'>
      {isLastCard ? (
        ...
        <div className='relative w-full max-w-[600px] md:max-w-[90vh]'>
          <FlashcardCard frontContent={currentCard?.question} backContent={currentCard?.answer} showAnswer={showAnswer} onClick={() => setShowAnswer(!showAnswer)} />
        </div>
      )}

      {!isLastCard && <Progress value={((currentIndex + 1) / flashcards.length) * 100} className='w-full max-w-md md:max-w-2xl' />}
          ...
          {Object.values(ReviewQuality)
            ...
            .map((quality) => (
              <Button
                ...
                size='sm'
                className={cn('px-2 py-1 text-xs md:px-4 md:py-2 md:text-sm', quality === ReviewQuality.Perfect && 'bg-green-600 hover:bg-green-700')}
              >
        ...
        <div className='flex w-full justify-center'>
          <div className='w-full max-w-2xl overflow-x-auto'>
            <div className='flex min-w-max justify-center'>
              <DockNavigate
                setCurrentIndex={(value: number | ((prev: number) => number)) => (typeof value === 'function' ? setCurrentIndex(value(currentIndex)) : setCurrentIndex(value))}
                setShowAnswer={(value: boolean | ((prev: boolean) => boolean)) => (typeof value === 'function' ? setShowAnswer(value(showAnswer)) : setShowAnswer(value))}
