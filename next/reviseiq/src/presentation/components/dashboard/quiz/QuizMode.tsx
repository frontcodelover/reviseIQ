...
export default function QuizMode() {
    ...
    const loadQuiz = async () => {
      try {
        ...
        if (result) {
          ...
          const shuffledAnswers = [...firstQuestion.wrongAnswers, firstQuestion.correctAnswer].sort(() => Math.random() - 0.5);
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestion < quiz.questions.length - 1) {
      ...
      const shuffledAnswers = [...nextQ.wrongAnswers, nextQ.correctAnswer].sort(() => Math.random() - 0.5);
      setAnswersOrder(shuffledAnswers);
    } else {
      setShowResult(true);
    }
              ...
              {answersOrder.map((answer, index) => {
                ...
                return (
                  <Button
                    ...
                    className={cn('group h-auto min-h-[60px] w-full justify-start p-4 text-left', 'transition-all duration-200', selectedAnswer && 'cursor-not-allowed', 'hover:bg-muted/50')}
                    onClick={() => handleAnswer(answer)}
                    disabled={selectedAnswer !== null}
                  >
                    <div className='flex w-full min-w-0 items-start gap-2'>
                      <div className='flex-shrink-0 pt-0.5'>{isSelected && (isCorrect ? <CheckCircle2 className='text-success h-5 w-5' /> : <XCircle className='h-5 w-5 text-destructive' />)}</div>
                      <div className='min-w-0 flex-1'>
                        <p className='whitespace-pre-wrap break-words text-sm leading-normal'>{answer}</p>
                      </div>
                    </div>
                  </Button>
                );
