...
export function Dashboard() {
  ...
  if (error) {
    return <div className='flex h-[50vh] items-center justify-center text-destructive'>{error}</div>;
  }
  ...
  return (
    <PageContainer>
      ...
      <LogsAndBadgesManager userId={userId} onLogsUpdate={setLogs} onBadgesUpdate={setBadges} onLastBadgeUpdate={setLastBadge} />
              ...
              <div className='space-y-2'>
                ...
                {lastBadge && (
                  <div className='flex items-center space-x-4 rounded-lg bg-white/10 p-4'>
                    <Image src={lastBadge.image_url} alt={lastBadge.name} width={64} height={64} className='object-contain' />
                    <div className='space-y-1'>
                      <h5 className='font-medium'>{lastBadge.name}</h5>
                      <p className='text-sm opacity-90'>{lastBadge.description}</p>
                      <p className='text-xs opacity-75'>Obtenu le {new Date(lastBadge.obtained_at).toLocaleDateString()}</p>
                    </div>
                  </div>
                )}
              </div>
