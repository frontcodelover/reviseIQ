...
function ActivityCalendar({ data }: ActivityCalendarProps) {
    ...
    if (view === 'month') {
      ...
      if (activityCount > 0) {
        return (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>{date.getDate()}</TooltipTrigger>
              <TooltipContent>
  ...
  return <Calendar value={date} tileClassName={getTileClassName} tileContent={getTileContent} className='rounded-lg border bg-card text-card-foreground shadow-sm' />;
}
