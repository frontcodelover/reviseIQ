...
describe('AuthService', () => {
  ...
  describe('signUp', () => {
    ...
    const mockError = new AuthError('Sign up error', 400)
		...
		await expect(authService.signUp('test@example.com', 'password')).rejects.toThrow('Sign up error')
	  })
	  ...
	  it('should throw an error if sign in fails', async () => {
		...
		await expect(authService.signInWithEmail('test@example.com', 'password')).rejects.toThrow('Sign in error')
	  })
	  ...
	  describe('signInWithProvider', () => {
		...
		it('should throw an error if provider sign in fails', async () => {
			...
			vi.spyOn(supabase.auth, 'signInWithOAuth').mockResolvedValue({ data: { provider: 'google', url: null }, error: mockError })
	  
			await expect(authService.signInWithProvider('google')).rejects.toThrow('Provider sign in error')
		  })
		})
		...
		describe('getUser', () => {
		  ...
		  it('should throw an error if getting user fails', async () => {
			...
			vi.spyOn(supabase.auth, 'getSession').mockResolvedValue({ data: { session: null }, error: mockError })
